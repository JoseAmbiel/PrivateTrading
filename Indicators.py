...
def separator(df_close, interv, sepa):
    if '_1' in interv:
        sep00_n = 48; sep01_n = 104; sep02_n = 20
        sep10_n = 48; sep11_n = 104
        sep00 = indct.EMA(df_close, 2/(sep00_n+1)) - indct.EMA(df_close, 2/(sep01_n+1))
        sep01 = indct.EMA(sep00, 2/(sep02_n+1))
        sep10 = indct.EMA(df_close, 2/(sep10_n+1))
        sep11 = indct.EMA(df_close, 2/(sep11_n+1))
        if sepa == 'uu':
            sep0 = np.greater_equal(sep01 - sep00, 0)
            sep1 = np.greater_equal(sep10 - sep11, 0)
        elif sepa == 'ud':
            sep0 = np.greater_equal(sep01 - sep00, 0)
            sep1 = np.less(sep10 - sep11, 0)
        elif sepa == 'du':
            sep0 = np.less(sep01 - sep00, 0)
            sep1 = np.greater_equal(sep10 - sep11, 0)
        elif sepa == 'dd':
            sep0 = np.less(sep01 - sep00, 0)
            sep1 = np.less(sep10 - sep11, 0)
    ...
    if '_2' in interv:
        ...
        sep000 = indct.RSI(df_close, sep000_n)
        sep00 = indct.EMA(sep000, 2/(sep00_n+1))
        sep01 = indct.EMA(sep000, 2/(sep01_n+1))
        sep10 = indct.EMA(df_close, 2/(sep10_n+1))
        sep11 = indct.EMA(df_close, 2/(sep11_n+1))
        if sepa == 'uu':
    ...
    if '_3' in interv:
        ...
        sep00 = indct.EMA(df_close, 2/(sep00_n+1))
        sep01 = indct.EMA(df_close, 2/(sep01_n+1))
        sep02 = indct.EMA(df_close, 2/(sep02_n+1))
        if sepa == 'abc':
            sep0 = np.greater_equal(sep00 - sep01, 0)
